...
/-!
# Browser comparison catalogue
-/

/-- Compiles every InfoView animation example for the JS/VIR comparison dashboard. -/
def animationExampleCatalogue : Array (String × CompiledAnimation) := #[
  ("Growing circle and color",
    compileAnimation
      [{ duration := 0.5 }, { duration := 1.0 }, { duration := 0.5 }]
      (fun progress =>
        let t := Easing.easeInOut progress[1]
  ...
  ("Bouncing ball and loop exit",
    compileAnimation
      ...
      (fun progress =>
        ...
        let shadow := Diagram.ellipse (12 * sx) 3
          (fill := .solid {
            color := { r := 0, g := 0, b := 0, a := 0.15 * (1 - arc * 0.7) * opacity }
          })
        Diagram.compose
          (Diagram.translate x 1.5 shadow)
          (fadeIn (Diagram.translate x (15 * sy + y) ball) opacity))),
  ("Cross-fade",
    compileAnimation
      ...
      (fun progress =>
        let t := Easing.easeInOut progress[1]
        let square := Diagram.rect 40 40
          (fill := .solid { color := Color.blue })
          (stroke := { color := Color.black, width := 1 })
        let circle := Diagram.circle 25
          (fill := .solid { color := Color.red })
          (stroke := { color := Color.black, width := 1 })
        crossFade square circle t)),
  ("Animated clipping",
    compileAnimation
      [{ duration := 2.0 }]
      (fun progress =>
  ...
  ("Nested named morph",
    compileAnimation
      ...
      (fun progress =>
        let (start, stop) := nestedMorphDiagrams
        let morph := start.morph stop
        morph.evaluate (Easing.easeInOut progress[0]))),
  ("Morphing arrows and final loop",
    compileAnimation
      [{ duration := 3 }, { duration := 2, loop := true }]
      (fun progress =>
        let (start, stop) := morphArrowTests
        let morph := start.morph stop
        let size := 1.5 - (0.25 * (-0.5 + progress[1]).abs)
        morph.evaluate (Easing.easeInOut progress[0]) |>.scale size)),
      ...
      (fun progress =>
        ...
        let ellipseShape := Diagram.ellipse 50 35
          (fill := .radialGradient
            ...
            (focal := ⟨fx, fy⟩))
          (stroke := { color := { r := 30, g := 60, b := 120 }, width := 2 })
        let dx := 30 * Float.sin θ
        let dy := 15 * Float.sin (2 * θ)
        Diagram.translate dx dy ellipseShape)),
  ("Animated styled text",
    compileAnimation
      [{ duration := 2.0 }]
      (fun progress =>
        let t := progress[0]
  ...
  ("Multiline text content",
    compileAnimation
      ...
      (fun progress =>
        let t := progress[0]
        let percent := (t * 100).round.toUInt64.toNat
        Diagram.text s!"Progress: {percent}%\nRemaining: {100 - percent}%" { fontSize := 14 })
      (fps := 30))
]
